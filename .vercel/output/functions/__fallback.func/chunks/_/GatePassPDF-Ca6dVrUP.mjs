...
import { updateDoc, doc, query, collection, orderBy, where, getDocs, setDoc } from 'firebase/firestore';
...
var gatePassService = {
  ...
  async getAllPasses(status) {
    ...
    if (status) q = query(collection(db, "gate_passes"), where("status", "==", status), orderBy("created_at", "desc"));
