...
if (typeof globalThis.addEventListener === "function") {
  globalThis.addEventListener("error", (event) => {
    var _a;
    return record((_a = event.error) != null ? _a : event);
  });
...
async function getServerEntry() {
  if (!serverEntryPromise) serverEntryPromise = import('./server-BaZlr8n8.mjs').then(function (n) { return n.s; }).then((m) => {
    var _a;
...
async function normalizeCatastrophicSsrResponse(response) {
  ...
  if (!((_a = response.headers.get("content-type")) != null ? _a : "").includes("application/json")) return response;
  const body = await response.clone().text();
  if (!body.includes('"unhandled":true') || !body.includes('"message":"HTTPError"')) return response;
  console.error((_b = consumeLastCapturedError()) != null ? _b : /* @__PURE__ */ new Error(`h3 swallowed SSR error: ${body}`));
...
const handlers = [
  { route: '/**', handler: server_default, lazy: false, middleware: false, method: undefined }
];
...
export { getRouteRulesForPath as g, parseQuery as p, renderErrorPage as r, toNodeListener as t, useNitroApp as u, withQuery as w };
