...
import { signInWithPopup, GoogleAuthProvider, createUserWithEmailAndPassword, updateProfile, sendPasswordResetEmail, signInWithEmailAndPassword } from 'firebase/auth';
...
function AuthPage() {
  ...
  useEffect(() => {
    if (!loading && user) navigate({ to: redirect != null ? redirect : "/tickets" });
  }, [
    user,
    loading,
    navigate,
