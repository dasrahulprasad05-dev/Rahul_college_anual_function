...
import { makeSerovalPlugin, defaultSerovalPlugins, parseRedirect, isRedirect, createSerializationAdapter, isResolvedRedirect, executeRewriteInput, createRawStreamRPCPlugin, invariant, isNotFound, resolveManifestAssetLink, resolveManifestCssLink, rootRouteId, getScriptPreloadAttrs, getStylesheetHref } from '@tanstack/router-core';
...
var manifest = {
  "a93969aae98d5e42b2b702cf4effb13c056aaf935b99873d1abd06d2c5aaa239": {
    functionName: "sendScanConfirmation_createServerFn_handler",
    importer: () => import('./ticket-emails-Dq6eMXs2.mjs')
  },
  "c0d8d04ebe61932401adcceeaa9c81f295042937a3a78c99ab31e9f621e45945": {
    functionName: "sendTicketConfirmation_createServerFn_handler",
    importer: () => import('./ticket-emails-Dq6eMXs2.mjs')
  }
};
      ...
      if (middlewareFn) {
        const userNext = async (userCtx = {}) => {
          ...
          const result2 = await callNextMiddleware({
            ...
            fetch: (_b = (_a2 = ctx._callSiteFetch) != null ? _a2 : userCtx.fetch) != null ? _b : ctx.fetch,
            result: userCtx.result !== void 0 ? userCtx.result : userCtx instanceof Response ? userCtx : ctx.result,
            error: (_c = userCtx.error) != null ? _c : ctx.error
          });
          if (result2.error)
            throw result2.error;
          return result2;
...
function serverFnBaseToMiddleware(options) {
  ...
  return {
    "~types": void 0,
    options: {
      inputValidator: (_a = options.validator) != null ? _a : options.inputValidator,
      client: async ({ next, sendContext, fetch, ...ctx }) => {
        ...
        return next(await ((_a2 = options.extractedFn) == null ? void 0 : _a2.call(options, payload)));
      },
...
async function getCsrfRequestValidationResult(opts, ctx) {
  ...
  if (fetchSite !== null)
    return matchValue((_a = opts.secFetchSite) != null ? _a : "same-origin", fetchSite, ctx);
...
async function getFailureResponse(opts, ctx) {
  ...
  return (_b = (_a = opts.failureResponse) == null ? void 0 : _a.clone()) != null ? _b : new Response("Forbidden", { status: 403 });
}
...
async function getStartManifest(matchedRoutes) {
  var _a;
  const { tsrStartManifest } = await import('./_tanstack-start-manifest_v-CV8SJDWL.mjs');
  ...
  for (const k in routes) {
    ...
    if ((_a = v.css) == null ? void 0 : _a.length)
      result.css = v.css;
...
function collectStaticHintsFromManifest(manifest, matchedRoutes) {
  ...
  for (const route of matchedRoutes) {
    ...
    for (const link of (_a = routeManifest.preloads) != null ? _a : []) {
      ...
      if (attrs.crossOrigin !== void 0)
        hint.crossOrigin = attrs.crossOrigin;
      hints.push(hint);
    }
    for (const link of (_b = routeManifest.css) != null ? _b : []) {
      const stylesheetHref = getStylesheetHref(link);
      if (((_c = manifest.inlineCss) == null ? void 0 : _c.styles[stylesheetHref]) !== void 0)
        continue;
...
function createEarlyHintsCollector(opts) {
  if (!(opts == null ? void 0 : opts.onEarlyHints) && !(opts == null ? void 0 : opts.responseLinkHeader))
    return;
  ...
  return {
    ...
    appendResponseHeaders: (headers) => {
      if (!(responseLinkHeaderEntries == null ? void 0 : responseLinkHeaderEntries.length))
        return;
...
async function transformManifestAssets(source, transformFn, _opts) {
  ...
  for (const route of Object.values(manifest.routes)) {
    if ((_a = route.preloads) == null ? void 0 : _a.length)
      route.preloads = await Promise.all(route.preloads.map(async (link) => {
    ...
    if (((_b = route.css) == null ? void 0 : _b.length) && !manifest.inlineCss)
      route.css = await Promise.all(route.css.map(async (link) => {
    ...
    if ((_c = route.scripts) == null ? void 0 : _c.length)
...
async function resolveInlineCssForRequest(opts) {
  var _a;
  if (opts.requestInlineCss !== void 0)
...
function warmupFinalManifest(opts) {
  ...
  if (opts.onError)
    warmupPromise.catch(opts.onError);
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import('./router-j_9pWM7h.mjs').then(function (n) { return n.r; }),
...
async function executeMiddleware(middlewares, ctx) {
  ...
  const disposeStreamResponse = async (reason) => {
    ...
    if (currentResponse === response.response || currentResponse instanceof Response && response.response.body !== null && currentResponse.body === response.response.body)
      ctx.response = void 0;
...
function createStartHandler(cbOrOptions) {
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      if ((router == null ? void 0 : router.serverSsr) && !responseOwnsCleanup)
...
async function handleRedirectResponse(response, request, getRouter) {
  ...
  if (opts.to && typeof opts.to === "string" && !opts.to.startsWith("/"))
    throw new Error(`Server side redirects must use absolute paths via the 'href' or 'to' options. The redirect() method's "to" property accepts an internal path only. Use the "href" property to provide an external URL. Received: ${JSON.stringify(opts)}`);
  if ([
    "params",
    "search",
    "hash"
  ].some((d) => typeof opts[d] === "function"))
    throw new Error(`Server side redirects must use static search, params, and hash values and do not support functional values. Received functional values for: ${Object.keys(opts).filter((d) => typeof opts[d] === "function").map((d) => `"${d}"`).join(", ")}`);
...
async function handleServerRoutes({ getRouter, request, url, executeRouter, context, executedRequestMiddlewares }) {
  ...
  if ((server == null ? void 0 : server.handlers) && isExactMatch) {
...
const serverBaZlr8n8 = /*#__PURE__*/Object.freeze({
	__proto__: null,
	createServerEntry: createServerEntry,
	default: server_default
});

export { TSS_SERVER_FUNCTION as T, createStart as a, createServerFn as b, createMiddleware as c, getServerFnById as g, serverBaZlr8n8 as s };
//# sourceMappingURL=server-BaZlr8n8.mjs.map
