...
type EventForm = { name: string; description: string; event_date: string; venue: string; is_published: boolean };
type EventRow = {
  id: string;
  name: string;
...
const emptyEvent: EventForm = { name: "", description: "", event_date: "", venue: "", is_published: false };
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <EventDialog
            ...
            trigger={<Button className="gradient-gold text-primary-foreground hover:opacity-90"><Plus className="w-4 h-4 mr-1.5" />New event</Button>}
          />
        </div>
        ...
        <div className="space-y-4">
          {!events?.length && (
            <div className="rounded-2xl border border-dashed border-border p-10 text-center text-muted-foreground">
              No events yet. Click <span className="text-foreground font-medium">New event</span> to create one.
            </div>
          )}
          {events?.map((e) => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${e.is_published ? "border-success/40 bg-success/10 text-success" : "border-border bg-muted text-muted-foreground"}`}>
                      {e.is_published ? "Published" : "Draft"}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">{new Date(e.event_date).toLocaleDateString()} · {e.venue ?? "—"}</p>
                  {e.description && <p className="text-sm text-muted-foreground mt-2 max-w-2xl">{e.description}</p>}
                </div>
                <div className="flex flex-wrap gap-2 justify-end">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => updateEvent.mutate({ id: e.id, vals: { is_published: !e.is_published } })}
                  >
                    {e.is_published ? <><EyeOff className="w-3.5 h-3.5 mr-1" />Unpublish</> : <><Eye className="w-3.5 h-3.5 mr-1" />Publish</>}
                  </Button>
                  <EventDialog
                    mode="edit"
                    initial={{
                    ...
                    trigger={<Button variant="ghost" size="icon"><Pencil className="w-4 h-4" /></Button>}
                  />
...
function EventDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (o) setForm(initial ?? emptyEvent); }}>
      <DialogTrigger asChild>{trigger}</DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{mode === "create" ? "Create event" : "Edit event"}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
          <div><Label>Description</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Date</Label><Input type="date" value={form.event_date?.slice(0, 10)} onChange={(e) => setForm({ ...form, event_date: e.target.value })} /></div>
